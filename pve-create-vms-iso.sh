...
function custom {
    ...
    read -p "Enter Storage or leave blank to default (local-lvm): " PROXMOXSTRG
...
function createvm {
    ...
    qm set ${VMID} --scsihw virtio-scsi-pci --scsi0 ${PROXMOXSTRG}:vm-${VMID}-disk-0 --ide2 ${PROXMOXSTRG}:cloudinit --boot c --bootdisk scsi0 --serial0 socket --vga serial0 1>/dev/null
}
function applycloudinit {
    qm set ${VMID} --cicustom "vendor=local:snippets/${CLOUDINITFILE}" 1>/dev/null
}
